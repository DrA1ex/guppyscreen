...
PrintStatusPanel::PrintStatusPanel(
    ...
    static lv_coord_t grid_main_row_dsc_detail[] = {
...
void PrintStatusPanel::reset() {
    ...
    filament_diameter = v.is_null() ? 1.750 : std::stod(v.get<std::string>());
    extruder_target = -1;
    heater_bed_target = -1;
...
void PrintStatusPanel::init(json &fans) {
    ...
    for (auto &f: fans.items()) {
        ...
        if (!fan_value.is_null()) {
            int v = static_cast<int>(fan_value.get<double>() * 100);
    ...
    if (!pstat_state.is_null()) {
        auto pstatus = pstat_state.get<std::string>();
...
void PrintStatusPanel::populate() {
    ...
    if (!printfile.is_null()) {
        const std::string fname = printfile.get<std::string>();
        if (fname.length() > 0) {
    ...
    if (!pstate.is_null() && pstate.get<std::string>() == "paused") {
    ...
    if (!v.is_null()) {
        int new_value = static_cast<int>(v.get<double>() * 100);
    ...
    if (!v.is_null()) {
        z_offset.update_label(fmt::format("{:.5} mm", v.get<double>()).c_str());
    }
}
...
void PrintStatusPanel::handle_metadata(const std::string &gcode_file, json &j) {
    ...
    if (!eta.is_null()) {
        estimated_time_s = static_cast<uint32_t>(eta.get<float>());
        ...
        if (!v.is_null()) {
            uint32_t passed = static_cast<uint32_t>(v.get<float>());
...
void PrintStatusPanel::consume(json &j) {
    ...
    if (!pstate.is_null()) {
        auto print_status = pstate.get<std::string>();
    ...
    if (!v.is_null()) {
        extruder_target = v.get<int>();
    }

    v = j["/params/0/heater_bed/target"_json_pointer];
    if (!v.is_null()) {
        heater_bed_target = v.get<int>();
    }

    v = j["/params/0/extruder/temperature"_json_pointer];
    if (!v.is_null()) {
        if (extruder_target > 0) {
            extruder_temp.update_label(fmt::format("{} / {}", v.get<int>(), extruder_target).c_str());
        } else {
            extruder_temp.update_label(fmt::format("{}", v.get<int>()).c_str());
        }
    }
    ...
    if (!v.is_null()) {
        if (heater_bed_target > 0) {
            bed_temp.update_label(fmt::format("{} / {}", v.get<int>(), heater_bed_target).c_str());
        } else {
            bed_temp.update_label(fmt::format("{}", v.get<int>()).c_str());
        }
    }
    ...
    if (!speed.is_null()) {
        int s = static_cast<int>(speed.get<double>());
    ...
    for (auto &f: fan_speeds) {
        ...
        if (!fan_value.is_null()) {
            fv = static_cast<int>(fan_value.get<double>() * 100);
            f.second = fv;
        }
    ...
    if (!current_file.is_null()) {
        ...
        if (!v.is_null()) {
            ...
            if (!first_layer_height.is_null() && !layer_height.is_null() && !object_height.is_null()) {
                auto layer = static_cast<int>(std::ceil(
                    (object_height.get<double>() - first_layer_height.get<double>()) / layer_height.get<double>()
                    + 1));
...
int PrintStatusPanel::current_layer(json &info) {
    ...
    if (!current_file.is_null()) {
        ...
        if (!pd.is_null()
            ...
            auto layer = static_cast<int>(std::ceil(
                (zpos.get<double>() - first_layer_height.get<double>()) / layer_height.get<double>() + 1));
