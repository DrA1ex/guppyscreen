...
MainPanel::MainPanel(
    ...
    , main_content(lv_obj_create(main_tab))
    , print_status_panel(websocket, lock, main_content)
    , print_panel(ws, lock, print_status_panel)
    , printertune_panel(ws, lock, printertune_tab, print_status_panel.get_finetune_panel())
    , numpad(Numpad(main_content))
    , extruder_panel(ws, lock, numpad, sm)
    , prompt_panel(websocket, lock, main_content)
    , spoolman_panel(sm)
    , temp_content(lv_obj_create(main_content))
    , temp_chart(lv_chart_create(main_content))
    , homing_btn(main_content, &move, "Homing", &MainPanel::_handle_homing_cb, this)
    , extrude_btn(main_content, &filament_img, "Extrude", &MainPanel::_handle_extrude_cb, this)
    , action_btn(main_content, &fan, "Fans", &MainPanel::_handle_fanpanel_cb, this)
    , led_btn(main_content, &light_img, "LED", &MainPanel::_handle_ledpanel_cb, this)
    , print_btn(main_content, &print, "Print", &MainPanel::_handle_print_cb, this)
    , status_btn(main_content, &info_img, "Status", &MainPanel::_handle_status_cb, this) {
...
void MainPanel::init(json &j) {
    ...
    for (const auto &el: sensors) {
        ...
        if (!target_value.is_null()) {
            int target = target_value.get<int>();
...
void MainPanel::consume(json &j) {
    ...
    auto &pstat_state = j["/printer_state/print_stats/state"_json_pointer];
    if (!pstat_state.is_null()) {
        if (pstat_state.get<std::string>() == "printing" || pstat_state.get<std::string>() == "paused") {
            status_btn.enable();
        } else {
            status_btn.disable();
        }
...
static void scroll_begin_event(lv_event_t *e) {
    ...
    if (lv_event_get_code(e) == LV_EVENT_SCROLL_BEGIN) {
        auto *a = (lv_anim_t *) lv_event_get_param(e);
        if (a) a->time = 0;
    }
}
...
void MainPanel::create_panel() {
    ...
    lv_obj_add_event_cb(lv_tabview_get_content(tabview), scroll_begin_event, LV_EVENT_SCROLL_BEGIN, nullptr);
...
void MainPanel::handle_status_cb(lv_event_t *event) {
    if (lv_event_get_code(event) == LV_EVENT_CLICKED) {
        spdlog::trace("clicked status");
...
void MainPanel::create_main(lv_obj_t *parent) {
    ...
    lv_obj_set_grid_dsc_array(main_content, grid_main_col_dsc, grid_main_row_dsc);

    lv_obj_set_style_pad_top(main_content, 4, 0);
    lv_obj_set_style_pad_bottom(main_content, 4, 0);
    ...
    lv_obj_set_grid_cell(status_btn.get_container(), LV_GRID_ALIGN_CENTER, 3, 1, LV_GRID_ALIGN_CENTER, 2, 1);

    lv_obj_clear_flag(temp_content, LV_OBJ_FLAG_SCROLLABLE);
    lv_obj_set_size(temp_content, LV_PCT(50), LV_PCT(62));
    lv_obj_set_style_pad_top(temp_content, 4, 0);
    lv_obj_set_style_pad_bottom(temp_content, 4, 0);
    lv_obj_set_style_pad_all(temp_content, 0, 0);

    lv_obj_set_flex_flow(temp_content, LV_FLEX_FLOW_ROW_WRAP);
    lv_obj_set_grid_cell(temp_content, LV_GRID_ALIGN_START, 0, 2, LV_GRID_ALIGN_CENTER, 0, 2);
...
void MainPanel::create_sensors(json &temp_sensors) {
    ...
    for (auto &sensor: temp_sensors.items()) {
        ...
        bool controllable = sensor.value()["controllable"].get<bool>();

        lv_color_t color_code = lv_palette_main(LV_PALETTE_ORANGE);
        if (!sensor.value()["color"].is_number()) {
            auto color = sensor.value()["color"].get<std::string>();
            if (color == "red") {
            ...
            color_code = lv_palette_main((lv_palette_t) sensor.value()["color"].get<int>());
        }

        auto display_name = sensor.value()["display_name"].get<std::string>();
        ...
        sensors.insert({
            key, std::make_shared<SensorContainer>(ws, temp_content, sensor_img, 150,
