...
HomingPanel::HomingPanel(KWebSocketClient &websocket_client, std::mutex &lock)
    ...
    , distance_selector(homing_cont, "Move Distance (mm)",
        {".1", ".5", "1", "5", "10", "25", "50", ""},
        2, 70, 15, &HomingPanel::_handle_selector_cb, this) {
    ...
    static lv_coord_t grid_main_row_dsc[] = {
        LV_GRID_FR(4), LV_GRID_FR(4), LV_GRID_FR(2), LV_GRID_TEMPLATE_LAST
    };
...
void HomingPanel::handle_callback(lv_event_t *event) {
    const char *distance = lv_btnmatrix_get_btn_text(distance_selector.get_selector(), distance_selector.get_selected_idx());
    std::string move_op;

    auto btn = lv_event_get_current_target(event);
    if (btn == home_all_btn.get_container()) {
        ...
        move_op = fmt::format("MOVE_SAFE Y=+{} F=6000", distance);
        ...
        move_op = fmt::format("MOVE_SAFE X=+{} F=6000", distance);
        ...
        move_op = fmt::format("MOVE_SAFE Z=+{} F=1800", distance);
...
void HomingPanel::update_buttons(const json &v) {
    if (!v.is_null()) {
        auto homed_axes = v.get<std::string>();
        ...
        if (homed_axes.find("xy") != std::string::npos) {
            home_xy_btn.disable();
        } else {
            home_xy_btn.enable();
        }

        if (homed_axes.find("xyz") != std::string::npos) {
