...
void PrintPanel::consume(json &j) {
    ...
    if (pstat_state.get<std::string>() != "printing"
        && pstat_state.get<std::string>() != "paused") {
        status_btn.disable();
    } else {
        status_btn.enable();
    }
...
void PrintPanel::subscribe() {
    ws.send_jsonrpc("server.files.list", R"({"root":"gcodes"})"_json, [this](json &d) {
        ...
        if (d.contains("result")) {
            for (auto f: d["result"]) {
                root.add_path(KUtils::split(f["path"], '/'), f["path"], f["modified"].get<uint32_t>());
            }
        }
...
void PrintPanel::foreground() {
    ...
    spdlog::debug("print panel print stats {}",
        pstat_state.is_null() ? "nil" : pstat_state.get<std::string>());

    if (!pstat_state.is_null()
        && pstat_state.get<std::string>() != "printing"
        && pstat_state.get<std::string>() != "paused") {
        status_btn.disable();
    } else {
        status_btn.enable();
