...
ExtruderPanel::ExtruderPanel(
    ...
    , extruder_temp(ws, panel_cont, &extruder, 150,
        "Extruder", lv_palette_main(LV_PALETTE_RED), false, true, numpad, "extruder", nullptr, nullptr)
    ...
    , load_material_btn(leftside_btns_cont, &load_filament_img, "Material", &ExtruderPanel::_handle_callback, this)
    ...
    if (!df.empty()) {
        ...
        if (!v.is_null()) {
            load_filament_macro = v.get<std::string>();
        }

        v = conf->get_json(conf->df() + "default_macros/unload_filament");
        if (!v.is_null()) {
            unload_filament_macro = v.get<std::string>();
        }

        v = conf->get_json(conf->df() + "default_macros/cooldown");
        if (!v.is_null()) {
            cooldown_macro = v.get<std::string>();
        }
    }
    ...
    static lv_coord_t grid_main_row_dsc[] = {
        LV_GRID_FR(3), LV_GRID_FR(6), LV_GRID_FR(6), LV_GRID_FR(6),
...
void ExtruderPanel::handle_callback(lv_event_t *e) {
    ...
    if (lv_event_get_code(e) == LV_EVENT_VALUE_CHANGED) {
        ...
        if (btn == load_material_btn.get_container()) {
            ws.gcode_script("LOAD_MATERIAL");
        }
